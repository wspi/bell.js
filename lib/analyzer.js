...
Analyzer.prototype.serve = function *() {
  ...
  while (1) {
    ...
    var end = new Date();
    log.info('Analyzed (%dms) %s: %s', end - start, name, key);
 ...
 *
 * Return the multiples of the deviation to 3 * singma.
 *
 * @param {Array} series
 * @return {Number}
 */
...
Analyzer.prototype.connectAlerter = function() {
  ...
  this.alerterConn = net.connect({port: port}, function(){
    ...
    log.warning('Socket error, alerter may be not up on port: %d', port);
