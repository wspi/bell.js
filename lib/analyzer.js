...
Analyzer.prototype.query = function *(datapoint) {  // jshint ignore: line
   ...
   * query current trend
   */
  var tset = prefix + 'trend';
  reqs.push(this.ssdb.zget(tset, name));
   ...
   * the current trend
   */
  // Note: ssdb currently only support integer as zset score
  var trend = +chunks.shift() / 1e5;
...
Analyzer.prototype.save = function *(datapoint, trend) {
  ...
  var resps = yield [
    ...
    // Note: ssdb currently only support integer as zset score
    this.ssdb.zset(tset, name, trend * 1e5)
  ];
