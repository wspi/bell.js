...
var secs2str = require('./util').secs2str;
var timespan2secs = require('./util').timespan2secs;
...
function *index(pattern) {
  ...
  var sort = this.request.query.sort === 'v' ? 'v' : 'm';
...
function *apiNames(pattern, limit, sort) {
  // cast parameters
  limit = +limit;
  sort = sort === 'v' ? 'value' : 'multi';

  // collect keys
  var zset = configs.ssdb.zset.trending[sort];
  var list = yield ssdb.zrscan(zset, '', '', '', -1);
  var keys = [];
