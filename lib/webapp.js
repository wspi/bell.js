...
var trendings;
// trending averages for dashboards
var trendingsAverages;
...
// @route apiDatapoints, '/api/tavg'
//
// request parameters:
//
//   @param {String} dashboard
function *apiTrendingAverage() {
  var dashboardSlug = this.request.query.dashboard;
  this.body = yield {time: trendingsAverages.time,
    data: trendingsAverages.data[dashboardSlug]};
}
...
exports = module.exports = {
  serve: function *() {
    ...
    if (cluster.isMaster) {
      ...
      app.use(route.get(url('/api/tavg'), apiTrendingAverage));
      // bind http server
      ...
      // start cron task to build trending averages
      yield  buildTrendingAverages();
      setInterval(co.wrap(buildTrendingAverages), 6 * configs.interval * 1e3);
    }
  }
};
...
// Build trending averages
function *buildTrendingAverages() {
  trendingsAverages = {time: +new Date() / 1000, data: {}}

  for (var slug in dashboards) {
    var keys = [];
    var dashboard = dashboards[slug];
    for (var i = 0; i < dashboard.patterns.length; i++) {
      [].push.apply(keys, filterTrendings(dashboard.patterns[i]));
    }

    var sum = 0;
    for (var key in trendings) {
      sum += +trendings[key].split(':')[0]
    }

    if (keys.length > 0) {
      trendingsAverages.data[slug] = sum / keys.length;
    } else {
      trendingsAverages.data[slug] = 0;
    }
    log.info('Build trending average done for dashborad slug %s', slug);
  }
}
