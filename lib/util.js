...
exports.log = bunyan.createLogger({name: 'bell', stream: process.stdout});
 ...
 * @return {Number}
 */
exports.timespan2secs = function(timespan) {
  var map = {
    's': 1,
    'm': 60,
    'h': 60 * 60,
  ...
  while (timespan.length > 0) {
    ...
    // return false if an illegal timespan
 ...
 * @return {String}
 */
exports.secs2str = function(secs) {
  var date = new Date(secs * 1000);
  var month = date.getMonth() + 1;  // getMonth() return 0~11 numbers
  var day = date.getDate();
  var hours = date.getHours();
  var minutes = date.getMinutes();
  var seconds = date.getSeconds();
  return format('%s/%s %s:%s:%s', month, day, hours, minutes, seconds);
};
